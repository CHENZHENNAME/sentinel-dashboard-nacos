...
app.controller('IdentityCtl', ['$scope', '$stateParams', 'IdentityService',
  ...
  "FlowNacosService","DegradeNacosService","ParamFlowNacosService","AuthorityRuleNacosService",
  '$interval', '$location', '$timeout',
  function ($scope, $stateParams, IdentityService, ngDialog,
    FlowService, DegradeService, AuthorityRuleService, ParamFlowService, MachineService, FlowNacosService, DegradeNacosService, ParamFlowNacosService
            , AuthorityRuleNacosService, $interval, $location, $timeout) {
    ...
    function saveFlowRuleNacos() {
      if (!FlowNacosService.checkRuleValid(flowRuleDialogScope.currentRule)) {
          return;
      }
      FlowNacosService.newRule(flowRuleDialogScope.currentRule).success(function (data) {
          if (data.code === 0) {
              flowRuleDialog.close();
              let url = '/dashboard/nacos/flow/' + $scope.app;
              $location.path(url);
          } else {
              alert('失败：' + data.msg);
          }
    ...
    //
      ...
      $scope.addNewAuthorityRuleNacos = function (resource) {
          if (!$scope.macInputModel) {
              return;
          }
          let mac = $scope.macInputModel.split(':');
          ...
          authorityRuleDialogScope.saveRule = saveAuthorityRuleNacos;
          authorityRuleDialogScope.saveRuleAndContinue = saveAuthorityRuleAndContinueNacos;
      ...
      function saveAuthorityRuleNacos() {
          let ruleEntity = authorityRuleDialogScope.currentRule;
          if (!AuthorityRuleNacosService.checkRuleValid(ruleEntity.rule)) {
              return;
          }
          AuthorityRuleNacosService.addNewRule(ruleEntity).success((data) => {
              if (data.success) {
                  authorityRuleDialog.close();
                  let url = '/dashboard/nacos/authority/' + $scope.app;
                  $location.path(url);
              } else {
                  alert('添加规则失败：' + data.msg);
              }
      ...
      function saveAuthorityRuleAndContinueNacos() {
          let ruleEntity = authorityRuleDialogScope.currentRule;
          if (!AuthorityRuleNacosService.checkRuleValid(ruleEntity.rule)) {
              return;
          }
      ...
      $scope.addNewParamFlowRuleNacos = function (resource) {
          if (!$scope.macInputModel) {
              return;
          }
          let mac = $scope.macInputModel.split(':');
          ...
          paramFlowRuleDialogScope.saveRule = saveParamFlowRuleNacos;
          paramFlowRuleDialogScope.saveRuleAndContinue = saveParamFlowRuleAndContinueNacos;
          // paramFlowRuleDialogScope.onOpenAdvanceClick = function () {
          //     paramFlowRuleDialogScope.paramFlowRuleDialog.showAdvanceButton = false;
          // };
          // paramFlowRuleDialogScope.onCloseAdvanceClick = function () {
      ...
      function saveParamFlowRuleNacos() {
          let ruleEntity = paramFlowRuleDialogScope.currentRule;
          if (!ParamFlowNacosService.checkRuleValid(ruleEntity.rule)) {
              return;
          }
          ParamFlowNacosService.addNewRule(ruleEntity).success((data) => {
              if (data.success) {
                  paramFlowRuleDialog.close();
                  let url = '/dashboard/nacos/paramFlow/' + $scope.app;
                  $location.path(url);
              } else {
                  alert('添加热点规则失败：' + data.msg);
              }
      ...
      function saveParamFlowRuleAndContinueNacos() {
          let ruleEntity = paramFlowRuleDialogScope.currentRule;
          if (!ParamFlowNacosService.checkRuleValid(ruleEntity.rule)) {
              return;
          }
